...
def reasoning_fn(state):
    ...
    Node–Edge Restriction Rule (Meaning-based)

    - 문장에서 ‘장소들 사이의 관계’를 설명하는 경우(예: 두 장소가 서로 연결됨, 통로, 사이, 이동 경로 등)는 edge_restrictions에 해당한다.
    → 두 장소를 연결하는 경로 자체가 제한되는 상황.

    - 문장에서 ‘각 개별 장소’에 대한 제한을 말하는 경우(예: 특정 구역은 UAV만, 어떤 방은 출입 금지 등)는 node_restrictions에 해당한다.
    → 장소 자체의 접근 제한.

    - 핵심 판단 기준은 특정 표현 패턴이 아니라, 문장이 설명하는 대상이
    “장소 간 연결(relationship)”인지  
    “단일 장소(area itself)”인지에 따라 결정한다.
    ...
    human_msg = HumanMessage(
        content=(
            ...
            "3) Determine static task_loss (destroyed/closed/unreachable).\n"
            "\n"
            "ONLINE mode (delta update)\n"
            "Dynamic Constraints (test_server에서 전달된 실시간 제약 정보):\n"
            f"{dynamic_text}\n\n"
            "1) 새로 들어온 dynamic constraints에 대한 reasoning 결과를 내어야한다."
            "2) 이를 위해 SceneGraph 를 함께 참고하여, 기존 제약/환경정보가 무엇인지와 새로 추가된 제약이 무엇인지 종합적으로 사고하라.\n"
            "3) SceneGraph 에 이미 반영된 기존 제약들은 그대로 두고, 새로운 dynamic constraint 로 인해 추가되거나 수정해야 하는 제약만 reasoning 결과로 출력하라.\n"
